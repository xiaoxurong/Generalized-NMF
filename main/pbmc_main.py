...
def arg_parser():
    ...
    parser.add_argument('--model', type=str, choices=['sscnmf', 'ricc', 'gnmf', 'gpcanmf', 'dscnmf', 'onmf', 'deepnmf', 'deepsscnmf', 'ssc-omp-nmf', 'oraclenmf'],
...
def main(model, r, K, sigma=0.0, alpha=0.1, l1_reg=0.01, random_state=None, max_iter=50, tol=1e-6, graph_weighting='dot-product', n_nonzero_coefs=8):
    # load pbmc data
    df = pd.read_csv('data/pbmc_10x_norm_common_sqrt_tp_trueID.txt', index_col=0, sep='\t')
